...
filename = 'spin_glass_gen_fields_vary_correlation_2017_08_15_1_54/correlated_driving_';
...
%                 load(char(strcat(filename, string(iter_4), '_', string(iter_5), '/data_', string(iter_6), '.mat')))
            ...
            interpolated = interp1(stats(1, t_index), stats(9, t_index), 0:1000:t_max);
...
final_stats = reshape(final_stats, [12, 30, 1]);
...
errorbar([1, .9, .7, .5, .3, .2, .1, .05, .03, .02, .01, 0], final_stats_mean, final_std, 'Color', [0, 0, 1])

figure(6)
hold on
histogram(final_stats(12, :), linspace(0, 2, 25), 'Normalization', 'probability')
histogram(final_stats(5, :), linspace(0, 2, 25), 'Normalization', 'probability')
histogram(final_stats(1, :), linspace(0, 2, 25), 'Normalization', 'probability')
