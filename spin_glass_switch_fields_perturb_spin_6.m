...
folder_name = strcat('spin_glass_switch_fields_min_model_gen_', string(date));
...
interaction_locs = 1 * (double(interaction_locs) .* (normrnd(0, 1.5, size(interaction_locs))));
...
%             driving_fields_2(rand_fast_spins, :) = normrnd(0, 20, [numel(rand_fast_spins), number_fields(iter_3)]);
            driving_fields_2(rand_fast_spins, :) = normrnd(0, 20, [numel(rand_fast_spins), 10001]);
            driving_fields_3(rand_fast_spins, :) = normrnd(0, 20, [numel(rand_fast_spins), 10001]);
