...
filename = 'spin_glass_switch_fields_drive_all_1024_spins_8_neighbors_2019_08_30_2_45/random_order_';
% instrinsic flip rates, two drives

plot_title = 'fixed';

mean_early_counts = zeros(1, 1024);
mean_late_counts = zeros(1, 1024);
mean_early_frac = zeros(1, 1024);
mean_late_frac = zeros(1, 1024);
mean_early_diss = zeros(1, 1024);
mean_late_diss = zeros(1, 1024);
all_early = [];
all_late = [];
spin_of_interest = 247;
flip_trajectory = zeros(1, 300);
local_energy_full_list = zeros(1024, 301);
local_energy_trajectory = zeros(1, 301);
local_energy_counts = zeros(1, 300);

mean_neighbors = zeros(1, 1024);
mean_neighbors_sqr = zeros(1, 1024);
mean_field = zeros(1, 1024);
mean_field_sqr = zeros(1, 1024);
max_field = zeros(1, 1024);
max_field_sqr = zeros(1, 1024);
field_var = zeros(1, 1024);
mean_flips = zeros(1, 1024);
mean_flips_sqr = zeros(1, 1024);
mean_early_flip_waits = zeros(1, 1024);
mean_late_flip_waits = zeros(1, 1024);
early_flip_waits_var = zeros(1, 1024);
late_flip_waits_var = zeros(1, 1024);
nz_early_flip_waits = zeros(1, 1024);
nz_late_flip_waits = zeros(1, 1024);
mean_early_order_var = zeros(1, 1024);
mean_late_order_var = zeros(1, 1024);
nz_early_order_var = zeros(1, 1024);
nz_late_order_var = zeros(1, 1024);
mean_early_order = zeros(1, 1024);
mean_late_order = zeros(1, 1024);

mean_neighbor_energy_i = zeros(1, 1024);
mean_neighbor_energy_f = zeros(1, 1024);
...
mean_flip_rate_difference = zeros(1024, 1);
...
%                     hist_2 = [hist_2, spin_hist];
            ...
            early_counts = zeros(1, 1024);
            late_counts = zeros(1, 1024);
            early_diss = zeros(1, 1024);
            late_diss = zeros(1, 1024);
            current_local_energy = state .* (interactions * state);
            local_energy_list = [current_local_energy];
            last_t = 0;
            
            early_flip_waits = zeros(1, 1024);
            late_flip_waits = zeros(1, 1024);
            early_flip_waits_sqr = zeros(1, 1024);
            late_flip_waits_sqr = zeros(1, 1024);
            
            early_order = zeros(1, 1024);
            late_order = zeros(1, 1024);
            early_order_sqr = zeros(1, 1024);
            late_order_sqr = zeros(1, 1024);
            
            neighbor_energy_init = zeros(1, 1024);
            for i = 1:num_spins
                neighbor_energy_init(i) = sum(current_local_energy(interactions(i, :) ~= 0));
            end
            
            flips_when_up = zeros(1024, 1);
            flips_when_down = zeros(1024, 1);
            time_when_up = zeros(1024, 1);
            time_when_down = zeros(1024, 1);
...
%             flip_rate_difference = flip_rate_difference .* state(late_count_indices);
            ...
            neighbor_energy_final = zeros(1, 1024);
            for i = 1:num_spins
...
histogram(flip_rate_differences(1024, :) * 100, 0:.25:10, 'FaceColor', 'r', 'Normalization', 'probability')
