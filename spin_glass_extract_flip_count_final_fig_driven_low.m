...
%             flip_order = flip_order(late_count_indices);
            ...
            for iter_6 = 1:31
                temp_energy_trajectory(iter_6) = temp_energy_trajectory(iter_6) + mean(local_energy_list(1:spin_of_interest, iter_6));
            end
...
% errorbar(10:10:300, flip_trajectory / num_extract / 1000, dev / 1000, 'x-', 'Color', 'b', 'LineWidth', 2)
hold on
plot(10:10:300, flip_trajectory / num_extract / 1000, 'Color', 'b', 'LineWidth', 2)
fill([10:10:300, 300:-10:10], [flip_trajectory / num_extract / 1000 + dev/1000, flip_trajectory(end:-1:1) / num_extract / 1000 - dev(end:-1:1)/1000], 'b', 'FaceAlpha', .4, 'EdgeColor', 'none')
xlabel('field switches', 'FontSize', 20, 'FontName', 'Helvetica Neue')
ylabel('flips/switch', 'FontSize', 20, 'FontName', 'Helvetica Neue', 'Color', 'b')
title('low flip rate average', 'FontSize', 20, 'FontName', 'Helvetica Neue')
axis([0, 300, 0, .0065])
...
for i = 1:length(ticklabels)
    if i == 1
        ...
        ticklabels_new{i} = ['\color{blue} 0.00' ticklabels{i}];
    end
end
