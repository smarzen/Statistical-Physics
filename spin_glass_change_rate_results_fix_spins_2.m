...
filename = 'spin_glass_switch_fields_min_model_cont_2018_05_31_12_49/min_model_';
...
for iter_4 = 1:10
    stat_means_i{iter_4} = zeros(1, numel(0:1000:t_max));
    stat_means_f{iter_4} = zeros(1, numel(0:100:t_max) - 1);
    stat_means_2_i{iter_4} = zeros(1, numel(0:1000:t_max));
    stat_means_2_f{iter_4} = zeros(1, numel(0:1000:t_max) - 1);
end
% stat_sq_mean = [];
...
%             load(char(strcat(filename, string(iter_4), '_', string(iter_5), '/extra_data.mat')))
            interp_stats = diff(interp1(stats(1, t_index), stats(5, t_index), 0:1000:t_max));
%              interp_stats = interp1(stats(1, t_index), stats(3, t_index), 0:1000:t_max);

            [temp, t_index, temp_2] = unique(stats_2(1, :));
            interp_stats_2 = diff(interp1(stats_2(1, t_index), stats_2(5, t_index), 0:1000:t_max));
...
%                     stat_means_i{iter_5} = stat_means_i{iter_5} + mean(reshape_stats(floor(size(reshape_stats, 1) / 2):end, :), 1);
                    stat_means_f{iter_5} = stat_means_f{iter_5} + mean(abs(diff(interp_hist, 1, 2)), 1);
