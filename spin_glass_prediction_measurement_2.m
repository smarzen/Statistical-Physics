...
filename = 'spin_glass_switch_fields_markov_order_2018_07_11_12_14/random_order_';
...
for iter_4 = 1:10
    stat_means_i{iter_4} = zeros(1, 3000);
    stat_means_f{iter_4} = zeros(1, 3000);
    stat_means_2_i{iter_4} = zeros(1, 3000);
...
%                 load(char(strcat(filename, string(iter_4), '_', string(iter_5), '/data_', string(iter_6), '.mat')))
                ...
                drive_energy = [drive_energy, next_drive_energy];
...
%              interp_stats = interp1(stats(1, t_index), stats(3, t_index), 0:1000:t_max);
...
% % %             
            interp_stats = interp1(stats(1, t_index), drive_energy(:, t_index)', 90:100:t_max);
            interp_stats = interp_stats';
            interp_stats = [drive_energy(:, 1), interp_stats];
            next_possible = [];
            other_drive = [];
            t = 1;
            all_fields = 1:11;
            while t < numel(0:100:t_max)
                possible = 1:3;
                if random_order(t) <= 2
                    possible(random_order(t)) = [];
                elseif random_order(t) == 3
                    possible = 4;
                elseif random_order(t) == 4
                    possible = 1:2;
                end
                next_possible = [next_possible, mean(interp_stats(possible, t), 1)];
%                 if random_order(t) == 5
%                     next_possible = [next_possible, interp_stats(6, t)];
%                 else
%                     next_possible = [next_possible, mean(interp_stats(1:, t), 1)];
%                 end
                possible = 1:4;
                possible(random_order(t)) = [];
                other_drive = [other_drive, mean(interp_stats(possible, t), 1)];
                t = t + 1;
                
%                 remaining = random_order(t:(t + number_fields(iter_5) - 1));
%                 used = [];
%                 other_drive_fields = all_fields;
%                 other_drive_fields(remaining) = [];
%                 for cycle_num = 1:number_fields(iter_5)
%                     if t < numel(0:100:t_max)
%                         next_possible = [next_possible, mean(interp_stats(remaining, t))];
%                         next_impossible = [next_impossible, mean(interp_stats(used, t))];
%                         other_drive = [other_drive, mean(interp_stats(other_drive_fields, t))];
%                         
%                         used = [used, remaining(1)];
%                         remaining(1) = [];
%                     end
%                     t = t + 1;
%                 end
...
%                     stat_means_i{iter_5} = stat_means_i{iter_5} + mean(reshape_stats(floor(size(reshape_stats, 1) / 2):end, :), 1);
                    stat_means_i{iter_5} = stat_means_i{iter_5} + next_possible;
%                     stat_means_f{iter_5} = stat_means_f{iter_5} + next_impossible;
                    stat_means_2_i{iter_5} = stat_means_2_i{iter_5} + other_drive;
                    stat_means_2_f{iter_5} = stat_means_2_f{iter_5} + mean(interp_stats, 1);
...
%     fit_1 = fminsearch(exp_fit, [2.5E5, .1, .5E5]);
    ...
    figure()
    plot(100:100:t_max, stat_means_i{iter_4}, 'LineWidth', 2)
    hold on
%     plot(100:100:t_max, stat_means_f{iter_4}, 'LineWidth', 2)
    plot(100:100:t_max, stat_means_2_i{iter_4}, 'LineWidth', 2)
%     plot(0:100:t_max, stat_means_2_f{iter_4}, 'LineWidth', 2)
    title(num2str(number_fields(iter_4)))
