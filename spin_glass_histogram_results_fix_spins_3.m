...
filename = 'spin_glass_switch_fields_min_model_cont_2018_05_29_2_56/min_model_';
...
%             interp_stats_2 = interp1(stats(1, t_index), stats(3, t_index), 0:100:t_max);
...
%                     stat_means_f{iter_3} = [stat_means_f{iter_3}, mean(interp_stats_2((end - 99):end))];
                    ...
                    stat_means_i{iter_5} = [stat_means_i{iter_5}, mean(interp_stats(1:100))];
                    stat_means_f{iter_5} = [stat_means_f{iter_5}, mean(interp_stats((901):(1000)))];
...
%                     
                    stat_means_2_i{iter_5} = [stat_means_2_i{iter_5}, mean(interp_stats_2(1:100))];
                    stat_means_2_f{iter_5} = [stat_means_2_f{iter_5}, mean(interp_stats_2((901):1000))];
...
% 
% filename = 'spin_glass_switch_fields_decorrelate_fields_2018_03_15_2_33/decorr_second_';
% % instrinsic flip rates, two drives
% 
% plot_title = 'fixed';
% 
...
%     subplot(3, 4, iter_4)
%     hold off
%     histogram(stat_means_i{iter_4}, -10:5:100, 'Normalization', 'probability')
%     hold on
%     histogram(stat_means_f{iter_4}, -10:5:100, 'Normalization', 'probability')
%     
%     figure(2)
%     subplot(3, 4, iter_4)
%     hold off
%     histogram(stat_means_2_i{iter_4}, -10:5:100, 'Normalization', 'probability')
%     hold on
%     histogram(stat_means_2_f{iter_4}, -10:5:100, 'Normalization', 'probability')
%     
%     figure(3)
%     subplot(3, 4, iter_4)
%     hold off
%     histogram(stat_means_f{iter_4} - stat_means_i{iter_4}, 15, 'Normalization', 'probability')
%     
%     figure(4)
%     subplot(3, 4, iter_4)
%     hold off
%     histogram(stat_means_2_f{iter_4} - stat_means_2_i{iter_4}, 15, 'Normalization', 'probability')
%     
%     figure(5)
%     subplot(3, 4, iter_4)
%     hold off
%     scatter(stat_means_i{iter_4}, stat_means_f{iter_4} - stat_means_i{iter_4}, 'filled')
%     
%     low_diss_fraction = [low_diss_fraction, sum(stat_means_f{iter_4} < 10) / numel(stat_means_2_f{iter_4})];
%     
%     figure(6)
%     hold on
%     h_1 = scatter(number_fields(iter_4), mean(stat_means_i{iter_4}), 'b', 'filled');
%     h_3 = scatter(number_fields(iter_4), mean(stat_means_i{iter_4}) + std(stat_means_i{iter_4})/10, 'b');
%     scatter(number_fields(iter_4), mean(stat_means_i{iter_4}) - std(stat_means_i{iter_4})/10, 'b')
%     h_2 = scatter(number_fields(iter_4), mean(stat_means_f{iter_4}), 'r', 'filled');
%     h_4 = scatter(number_fields(iter_4), mean(stat_means_f{iter_4}) + std(stat_means_f{iter_4})/10, 'r');
%     scatter(number_fields(iter_4), mean(stat_means_f{iter_4}) - std(stat_means_f{iter_4})/10, 'r')
%     
%     xlabel('number of fields in $M$', 'FontSize', 20, 'Interpreter', 'LaTeX')
%     ylabel('work absorption rate', 'FontSize', 20, 'Interpreter', 'LaTeX')
%     title('work absorption rate over time', 'FontSize', 20, 'Interpreter', 'LaTeX')
%     legend([h_1, h_3, h_2, h_4], {'initial work absorption rate', 'initial st.dev.', 'final work absorption rate', 'final st.dev.'}, 'Interpreter', 'LaTeX', 'FontSize', 16)
%     
%     
%     figure(7)
%     hold on
%     h_5 = scatter(number_fields(iter_4), mean(stat_means_f{iter_4}), 'r', 'filled');
%     h_7 = scatter(number_fields(iter_4), mean(stat_means_f{iter_4}) + std(stat_means_f{iter_4})/10, 'r');
%     scatter(number_fields(iter_4), mean(stat_means_f{iter_4}) - std(stat_means_f{iter_4})/10, 'r')
%     h_6 = scatter(number_fields(iter_4), mean(stat_means_2_i{iter_4}), 'k', 'filled');
%     h_8 = scatter(number_fields(iter_4), mean(stat_means_2_i{iter_4}) + std(stat_means_i{iter_4})/10, 'k');
%     scatter(number_fields(iter_4), mean(stat_means_2_i{iter_4}) - std(stat_means_i{iter_4})/10, 'k')
%     
%     xlabel('number of fields in $M$ and $M''$', 'FontSize', 20, 'Interpreter', 'LaTeX')
%     ylabel('work absorption rate', 'FontSize', 20, 'Interpreter', 'LaTeX')
%     title('work absorption rate when changing drives', 'FontSize', 20, 'Interpreter', 'LaTeX')
%     legend([h_5, h_7, h_6, h_8], {'final $M$ work absorption rate', 'final $M$ st.dev.', 'initial $M''$ work absorption rate', 'initial $M''$ st.dev.'}, 'Interpreter', 'LaTeX', 'FontSize', 16)
    
    covar = cov(stat_means_f{iter_4}, stat_means_2_f{iter_4});
    r_vals = [r_vals, covar(1, 2) / sqrt(covar(1, 1) * covar(2, 2))];

    figure(8)
    subplot(2, 3, iter_4)
    scatter(stat_means_f{iter_4}, stat_means_2_f{iter_4}, 'filled', 'MarkerFaceColor', [iter_4 / 5, 0, 1 - iter_4 / 5], 'MarkerEdgeColor', [iter_4 / 5, 0, 1 - iter_4 / 5])
    xlabel('internal energy', 'FontSize', 20)
    ylabel('final work abs rate', 'FontSize', 20)
    title(strcat(num2str(number_fields(iter_4)), ' fields'), 'FontSize', 20)
    legend({strcat('r = ', num2str(covar(1, 2) / sqrt(covar(1, 1) * covar(2, 2))))}, 'FontSize', 16)
