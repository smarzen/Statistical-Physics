...
folder_name = strcat('spin_glass_switch_fields_fix_slow_strong_fields_', string(date));
...
%         final_spins = [final_spins, spin_glass_function_switch_driving_fields_fast(initial_spin_list(:, iter_2), interactions, spin_barriers, 0, 10, zeros(num_spins, 1), 100, true, 0, 100000, strcat(folder_name, '/', file))];
        final_spins = [final_spins, spin_glass_function_switch_driving_fields_fix_spins(initial_spin_list(:, iter_2), logical(zeros(num_spins, 1)), interactions, spin_barriers, 0, 10, zeros(num_spins, 1), 100, zeros([1, 10001]), 0, 100000, strcat(folder_name, '/', file))];
    end
end
...
%         driving_fields(rand_fast_spins, :) = normrnd(0, 2, [numel(rand_fast_spins), 1]);
        driving_fields(rand_fast_spins, :) = normrnd(0, 20, [numel(rand_fast_spins), num_fields]);
...
%             final_spins_fixed = spin_glass_function_switch_driving_fields_fix_spins(final_spins(:, 1), logical(zeros(num_spins, 1)), interactions, spin_barriers, 0, 10, driving_fields_2, 20000, random_order, 0, 100000, strcat(folder_name, '/', file_5));
            ...
            final_spins_fixed = spin_glass_function_switch_driving_fields_fix_spins(final_spins(:, iter_3), logical(zeros(num_spins, 1)), interactions, spin_barriers, 0, 5, driving_fields, 10, random_order, 0, 100000, strcat(folder_name, '/', file));
...
%             
            ...
            final_spins_fixed = spin_glass_function_switch_driving_fields_fix_spins(final_spins(:, iter_3), logical(zeros(num_spins, 1)), interactions, spin_barriers, 0, 5, zeros(num_spins, 1), 10, random_order, 0, 100000, strcat(folder_name, '/', file_2));
% %             fixed_spins = zeros(num_spins, 1);
