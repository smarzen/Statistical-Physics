...
filename = 'spin_glass_switch_fields_drive_quarter_reverse_field_order_2019_02_12_4_56/random_order_';
...
mean_work_abs = zeros(1, 590);
mean_other_abs = zeros(1, 590);
mean_next_diss = zeros(1, 590);
mean_other_diss = zeros(1, 590);
...
%             file_list = dir(char(strcat(filename, string(iter_4), '_', string(iter_5))));
            ...
            flips_2 = [];
            spin_diss_2 = [];
            for iter_6 = 1:(numel(file_list) - 4)
...
%                     hist_2 = [hist_2, spin_hist];
            ...
            load(char(strcat(filename, string(iter_3), '_', string(iter_4), '_', string(iter_5), '/trans.mat')))
            ...
            next_diss_trace = [];
            other_diss_trace = [];
%             transitions = zeros(size(driving_fields, 2));
%             for field_num = 1:290
%                 transitions(random_order(field_num), random_order(field_num + 1)) = transitions(random_order(field_num), random_order(field_num + 1)) + 1;
%             end
%             transitions(transitions ~= 0) = 1;
            ...
            for iter_7 = 1:590
                ...
                next_diss = 0;
                other_diss = 0;
                current_field = random_order(iter_7);
                for field_num = 1:10
                    if field_num ~= current_field
                        [potential_work, potential_diss] = spin_glass_function_switch_driving_fields_return_work(state, logical(zeros(num_spins, 1)), interactions, spin_barriers, 0, 10, driving_fields(:, field_num), 100, ones(1, 2), 0, 100, 'something_went_wrong');
                        potential_work = sum(state .* (driving_fields(:, field_num) - driving_fields(:, current_field)));
                        if transitions(random_order(iter_7), field_num)
                            next_energy = next_energy + potential_work;
                            next_diss = next_diss + potential_diss;
                        else
                            other_energy = other_energy + potential_work;
                            other_diss = other_diss + potential_diss;
                        end
                    end
                end
                if isnan(next_diss) || isnan(other_diss)
                    disp('sim error');
                end
                
                
                if transitions(current_field, current_field)
                    if next_energy == 0
                        work_abs = [work_abs, 0];
                    else
                        work_abs = [work_abs, next_energy / (sum(transitions(random_order(iter_7), :)) - 1)];
                    end
                    if isnan(next_diss / (sum(transitions(random_order(iter_7), :)) - 1))
                        disp('calc error');
                    end
                    next_diss_trace = [next_diss_trace, next_diss / (sum(transitions(random_order(iter_7), :)) - 1)];
                    other_abs = [other_abs, other_energy / sum(1 - transitions(random_order(iter_7), 1:6))];
                    other_diss_trace = [other_diss_trace, other_diss / sum(1 - transitions(random_order(iter_7), 1:10))];
                else
                    if isnan(other_diss / (sum(1 - transitions(random_order(iter_7), 1:10)) - 1))
                        disp('calc error 2');
                    end
                    work_abs = [work_abs, next_energy / sum(transitions(random_order(iter_7), :))];
                    other_abs = [other_abs, other_energy / (sum(1 - transitions(random_order(iter_7), 1:10)) - 1)];
                    next_diss_trace = [next_diss_trace, next_diss / sum(transitions(random_order(iter_7), :))];
                    other_diss_trace = [other_diss_trace, other_diss / (sum(1 - transitions(random_order(iter_7), 1:10)) - 1)];
                end
            end
%             
%             figure(31)
...
%             plot(revisit_snaps)
            ...
            mean_work_abs = mean_work_abs + work_abs;
            mean_other_abs = mean_other_abs + other_abs;
            mean_next_diss = mean_next_diss + next_diss_trace;
            mean_other_diss = mean_other_diss + other_diss_trace;
            if any(isnan(mean_next_diss))
                disp('add error');
            end
...
figure()
plot(mean_work_abs / 10000, 'LineWidth', 2)
hold on
plot(mean_other_abs / 10000, 'LineWidth', 2)
xlabel('t', 'FontSize', 20)
ylabel('number state', 'FontSize', 20)

figure()
plot(mean_next_diss / 10000, 'LineWidth', 2)
hold on
plot(mean_other_diss / 10000, 'LineWidth', 2)
xlabel('t', 'FontSize', 20)
ylabel('number state', 'FontSize', 20)
