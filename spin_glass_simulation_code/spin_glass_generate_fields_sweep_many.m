...
folder_name = strcat('spin_glass_gen_fields_vary_correlation_', string(date));
...
driving_fields = zeros(num_spins, 150);
rand_fast_spins = fast_spins(randperm(numel(fast_spins), numel(fast_spins)/2));
driving_fields(rand_fast_spins, :) = normrnd(0, 2, [numel(rand_fast_spins), size(driving_fields, 2)]);
% random_order = randi(size(driving_fields, 2) - 1, 10001, 1);
...
%             file = strcat('periodic_driving_', string(iter), '_', string(iter_2), '_', string(iter_3));
            file_2 = strcat('uncorrelated_driving_', string(total_iter), '_', string(iter_2), '_', string(iter_3));
            file_3 = strcat('correlated_driving_', string(total_iter), '_', string(iter_2), '_', string(iter_3));
...
%             final_spins_random = spin_glass_function_switch_driving_fields_random_fast(final_spins(:, iter_3), interactions, spin_barriers, 0, 10, driving_fields(:, 1:number_fields(iter_2)), 100, random_order, 0, 1000000, strcat(folder_name, '/', file_2));
            final_spins_generate = spin_glass_function_generate_driving_fields_fast_4(final_spins(:, iter_3), interactions, spin_barriers, 0, 10, 2, 100, driving_fields(:, 1:3), deterministic_probs(iter_2), 0, 1000000, strcat(folder_name, '/', file_2));
            final_spins_generate_2 = spin_glass_function_generate_driving_fields_fast_3(final_spins(:, iter_3), interactions, spin_barriers, 0, 10, 2, 100, driving_fields(:, 1:3), deterministic_probs(iter_2), 0, 1000000, strcat(folder_name, '/', file_3));
        end
    end
end
