function [final_spins, total_heat_lost] = spin_glass_function_switch_driving_fields_calc_corr(spins, fixed_spins, interactions, spin_barriers, external_field, beta, driving_fields, switch_time, random_order, driving_strength, t_max, file_name)
...
total_work = sum(driving_energy);
total_spin_work = driving_energy;
...
%             force_vec = zeros([num_spins, 1]);
    ...
    %             spin_work_hist = [spin_work_hist, total_spin_work];
            ...
            if (t >= 20000 && t <= 30000) || (t >= 50000 && t <= 60000) || (t >= 80000 && t <= 90000)
                spin_hist = [spin_hist, spins];
            end
            if t == 30000
                for corr_time = corr_times
                    correlation = mean(spin_hist(:, 1:(end - corr_time)) .* spin_hist(:, (1 + corr_time):end), 2);
                    spin_corrs_1 = [spin_corrs_1, correlation];
                end
                spin_hist = [];
            end
            if t == 60000
                for corr_time = corr_times
                    correlation = mean(spin_hist(:, 1:(end - corr_time)) .* spin_hist(:, (1 + corr_time):end), 2);
                    spin_corrs_2 = [spin_corrs_2, correlation];
                end
                spin_hist = [];
            end
            if t == 90000
                for corr_time = corr_times
                    correlation = mean(spin_hist(:, 1:(end - corr_time)) .* spin_hist(:, (1 + corr_time):end), 2);
                    spin_corrs_3 = [spin_corrs_3, correlation];
                end
                spin_hist = [];
            end
...
%                 neighbor_dist = [neighbor_dist, new_neighbor_dist];
    ...
    %                     end
                ...
                flip_times = [flip_times, t];
                flip_order = [flip_order, flip_spin];
