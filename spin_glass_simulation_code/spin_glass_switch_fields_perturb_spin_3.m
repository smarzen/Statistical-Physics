...
folder_name = strcat('spin_glass_switch_fields_min_model_', string(date));
...
number_fields = [2, 3, 5, 10, 20, 30, 50, 100, 200, 300];
...
%         semi_random_order(randperm(numel(semi_random_order), floor(numel(semi_random_order) / 10))) = 2;
        ...
        for iter_3= 1:10
            driving_fields = zeros(num_spins, number_fields(iter_3));
            driving_fields_2 = zeros(num_spins, number_fields(iter_3));
            
            driving_fields(rand_fast_spins, :) = normrnd(0, 20, [numel(rand_fast_spins), number_fields(iter_3)]);
...
%             random_order_b = random_order(randperm(numel(random_order)));
            ...
            random_order_2 = [];
            while numel(random_order_2) < 10001
                random_order_2 = [random_order_2, randperm(number_fields(iter_3))];
            end
            random_order_2 = diff(random_order_2);
...
%             modified_spins(1) = -1 * modified_spins(1);
            ...
            final_spins_fixed_2 = spin_glass_function_switch_driving_fields_fix_spins(modified_spins, logical(fixed_spins), interactions, spin_barriers, 0, 10, driving_fields_2, 100, random_order_2, 0, 100000, strcat(folder_name, '/', file_5));
%             
