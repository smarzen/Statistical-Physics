...
filename = 'spin_glass_switch_fields_drive_all_switch_drives_2019_04_01_3_10/random_order_';
...
%     stat_means_f{iter_4} = zeros(1, 600);
    ...
    all_stats_beg{iter_4} = [];
    all_stats_end{iter_4} = [];
end
% stat_sq_mean = [];
...
%               interp_stats = interp1(stats(1, t_index), stats(2, t_index) - stats(3, t_index), 10:100:t_max);
%              interp_stats = interp_stats(1:(end - 1)) .* interp_stats(2:end) / mean(interp_stats .^2);

            [temp, t_index, temp_2] = unique(stats_2(1, :));
            interp_stats_2 = interp1(stats_2(1, t_index), stats(2, t_index) - stats(3, t_index), 0:100:t_max);
            
            all_stats_beg{iter_5} = [all_stats_beg{iter_5}, interp_stats_2(5)];
            all_stats_end{iter_5} = [all_stats_end{iter_5}, interp_stats(5)];
...
%     xlabel('Number of field switches', 'FontSize', 20)
%     ylabel('Work absorption rate', 'FontSize', 20)
%     title('Work absorption rate when switching field order', 'FontSize', 20)
%     leg = legend({'5', '8', '12'}, 'FontSize', 20);
%     title(leg, 'Number of driving fields')

    figure()
    scatter(all_stats_beg{iter_4}, all_stats_end{iter_4}, 'Filled')
    xlabel('field energy')
    ylabel('work abs rate')
