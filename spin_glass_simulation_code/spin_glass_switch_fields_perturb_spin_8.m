...
folder_name = strcat('spin_glass_switch_fields_min_model_permute_', string(date));
...
%         final_spins = [final_spins, spin_glass_function_switch_driving_fields_fast(initial_spin_list(:, iter_2), interactions, spin_barriers, 0, 10, zeros(num_spins, 1), 100, true, 0, 100000, strcat(folder_name, '/', file))];
        final_spins = [final_spins, spin_glass_function_switch_driving_fields_fix_spins(initial_spin_list(:, iter_2), logical(zeros(num_spins, 1)), interactions, spin_barriers, 0, 10, zeros(num_spins, 1), 100, zeros(1, 10000), 0, 100000, strcat(folder_name, '/', file))];
    end
end
