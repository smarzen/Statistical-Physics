function final_spins = spin_glass_function_generate_driving_fields_fast_3(spins, interactions, spin_barriers, external_field, beta, driving_field_mag, switch_time, deterministic_fields, p_deterministic, driving_strength, t_max, file_name)
...
%                 driving_field = driving_fields(:, field_number);
                ...
                if field_number > 0 || rand(1) < p_deterministic
                    ...
                    driving_new_field = [driving_new_field, field_number];
                    if field_number ~= 1 && field_number ~= size(deterministic_fields, 2)
                        driving_field = zeros(num_spins, 1);
                        rand_fast_spins = fast_spins(randperm(numel(fast_spins), numel(fast_spins)/2));
                        driving_field(rand_fast_spins) = normrnd(0, driving_field_mag, [numel(rand_fast_spins), 1]);
                    else
                        driving_field = deterministic_fields(:, field_number);
                        if field_number == size(deterministic_fields, 2)
                            field_number = 0;
                        end
                    end
