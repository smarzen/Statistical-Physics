function final_spins = spin_glass_function_generate_driving_fields_fast_4(spins, interactions, spin_barriers, external_field, beta, driving_field_mag, switch_time, deterministic_fields, p_deterministic, driving_strength, t_max, file_name)

% randomly adds a deterministic field
...
driving_field = zeros(num_spins, 1);
rand_fast_spins = fast_spins(randperm(numel(fast_spins), numel(fast_spins)/2));
driving_field(rand_fast_spins) = normrnd(0, driving_field_mag, [numel(rand_fast_spins), 1]);
...
%             flip_counts = [flip_counts, flip_count];
            ...
            if iterations_since_last_save >= iterations_per_save
                save(char(strcat(file_name, '/data_', num2str(file_num))), 'statistics', 'spin_dissipation')%, 'spin_hist', 'flip_counts', 'spin_dissipation')
                iterations_since_last_save = 0;
                statistics = [];
                spin_hist = [];
                spin_dissipation = [];
...
%                 delta_t = (switch_time - mod(t, switch_time));
                ...
                driving_change = - sum(driving_energy);
                if numel(driving_new_field) > 0
                    driving_old_field = [driving_old_field, driving_new_field(end)];
                end
                driving_energy(:) = 0;
...
%                 if is_periodic
%                     field_number = mod(field_number, size(driving_fields, 2)) + 1;
%                 else
%                     other_field_numbers = 1:size(driving_fields, 2);
%                     other_field_numbers(field_number) = [];
%                     field_number = other_field_numbers(randi(numel(other_field_numbers)));
%                 end

%                 driving_field = driving_fields(:, field_number);
                
                if field_number > 0 || rand(1) < p_deterministic
                    if field_number == 1
                        field_number = 0;
                        ...
                        rand_fast_spins = fast_spins(randperm(numel(fast_spins), numel(fast_spins)/2));
                        driving_field(rand_fast_spins) = normrnd(0, driving_field_mag, [numel(rand_fast_spins), 1]);
                        driving_new_field = [driving_new_field, 0];
                    else
                        field_number = 1;

                        if rand(1) < .5
                            driving_new_field = [driving_new_field, 1];
                            driving_field = deterministic_fields(:, 1);
                        else
                            driving_new_field = [driving_new_field, size(deterministic_fields, 2)];
                            driving_field = deterministic_fields(:, size(deterministic_fields, 2));
                        end
                    end
                ...
                driving_energy = spins .* driving_field;
                driving_change = driving_change + sum(driving_energy);
                driving_changes = [driving_changes, driving_change];
                energy = energy + sum(driving_energy);
                total_work = total_work + sum(driving_energy);
                total_spin_work = total_spin_work + driving_energy;
...
% driving_times_save = driving_times;
% save(char(file_name));
save(char(strcat(file_name, '/data_', num2str(file_num))), 'statistics', 'spin_hist', 'flip_counts', 'spin_dissipation')
