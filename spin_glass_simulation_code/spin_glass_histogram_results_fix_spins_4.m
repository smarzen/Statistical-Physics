...
% filename = 'spin_glass_switch_fields_fix_slow_turn_off_drive_2018_01_24_4_44/unfixed_slow_';
...
filename = 'spin_glass_switch_fields_fix_slow_strong_fields_2018_02_14_12_34/fixed_slow_';
filename_2 = 'spin_glass_switch_fields_fix_slow_strong_fields_2018_02_14_12_34/driving_disabled_';
% instrinsic flip rates, two drives
...
%                 load(char(strcat(filename, string(iter_3), '_', string(iter_4), '_', string(iter_5), '/data_', string(iter_6), '.mat')))
                load(char(strcat(filename_2, string(iter_3), '_', string(iter_5), '/data_', string(iter_6), '.mat')))
                stats = [stats, statistics];
                flips = [flips, flip_counts];
                hist = [hist, spin_hist];
...
%                     hist_2 = [hist_2, spin_hist];
                ...
                load(char(strcat(filename, string(iter_3), '_', string(iter_4), '_', string(iter_5), '/data_', string(iter_6), '.mat')))
                stats_3 = [stats_3, statistics];
                hist_3 = [hist_3, spin_hist];
                flips_3 = [flips_3, flip_counts];
...
%             load(char(strcat(filename, string(iter_4), '_', string(iter_5), '/extra_data.mat')))
%             interp_stats = diff(interp1(stats(1, t_index), stats(5, t_index), 0:100:t_max));
            interp_stats_3 = diff(interp1(stats(1, t_index), stats_3(5, t_index), 0:100:t_max));
%             [temp, t_index, temp_2] = unique(stats_2(1, :));
%             interp_stats_2 = diff(interp1(stats_2(1, t_index), stats_2(5, t_index), 0:100:t_max));
%             interp_stats_4 = interp1(stats_2(1, t_index), stats_2(3, t_index), 0:100:t_max);
...
%             fast_flips_f = mean(interp_flips((floor(num_spins / 2) + 1):end, 4950:5050));
            ...
            if iter_5 > 0
                if iter_5 <= 100 %&& has_flips(iter_3, iter_4, iter_5) == 1
%                     stat_means_i{1} = [stat_means_i{1}, mean(interp_stats_2((end - 199):(end - 99)))];
%                     stat_means_i_2{iter_3} = [stat_means_i_2{iter_3}, sum(flips(1:32, end) - flips(1:32, (end - 1000)))];%
%                     stat_means_f_2{iter_3} = [stat_means_f_2{iter_3}, sum(flips_2(1:32, end) - flips_2(1:32, (end - 1000)))];%
...
%                     stat_means_i_2{iter_3} = [stat_means_i_2{iter_3}, mean(abs(mean(interp_stats_2(:, (end - 99):end), 2) - mean(interp_stats(:, (end - 99):end), 2)))];
                    
%                     stat_means_i_2{iter_3} = [stat_means_i_2{iter_3}, mean(interp_stats_3(1:10))];
%                     stat_means_f_2{iter_3} = [stat_means_f_2{iter_3}, mean(interp_stats_3((end - 9):end))];
                    
%                     stat_means_i_2{iter_3} = [stat_means_i_2{iter_3}, mean(interp_stats_3(1:100))];
%                     stat_means_f_2{iter_3} = [stat_means_f_2{iter_3}, mean(interp_stats_3((end - 99):end))];
...
%     all_means_2_f_2 = [all_means_2_f_2, stat_means_2_f_2{iter_4}];
    ...
    i_means = stat_means_i_2{iter_4};
    f_means = stat_means_f_2{iter_4};

    figure(5)
    subplot(3, 4, iter_4)
    histogram(mean(abs(f_means - i_means)), 0:.03:1.5, 'Normalization', 'probability')

    figure(6)
    subplot(3, 4, iter_4)
    histogram(mean(abs(f_means - i_means(:, randperm(size(i_means, 2))))), 0:.03:1.5, 'Normalization', 'probability')
...
% 
% figure(4)
% scatter(all_means_i_2, all_means_f_2, 'Filled')

% figure(37)
% scatter(all_means_i_2, all_means_f_2, 'Filled')

% figure(14)
% scatter(all_means_i_2, all_means_f_2)
% 
% figure(15)
% histogram(all_means_f_2, 200)

% figure(1)
% scatter(all_means_f, all_means_i, 'Filled')
% xlabel('internal energy', 'FontSize', 20)
% ylabel('work absorption rate', 'FontSize', 20)
% title('energy vs. work absorption', 'FontSize', 20)
% 
% figure(2)
% scatter(all_means_f, all_means_i_2 - all_means_i, 'Filled')
% 
% figure(3)
% scatter(all_means_f, all_means_f_2 - all_means_i, 'Filled')
% 
% bucket_size = 1;
% bucket_min = -1;
...
% xlabel('work absorption before freezing', 'FontSize', 20)
% ylabel('work absorption after freezing', 'FontSize', 20)
% title('same drive after freezing slow spins', 'FontSize', 20)
% 
% figure(4)
% scatter(all_means_i, all_means_i_2, 'Filled')
% xlabel('work absorption before freezing', 'FontSize', 20)
% ylabel('work absorption after freezing', 'FontSize', 20)
% title('different drive after freezing slow spins', 'FontSize', 20)
% 
% figure(10)
...
% figure(7)
% hold off
% histogram(mean(abs(all_means_f - all_means_i)), 0:.02:2, 'Normalization', 'probability')
% hold on
% histogram(mean(abs(all_means_i_2 - all_means_i)), 0:.02:2, 'Normalization', 'probability')
% xlabel('change in mean spin configuration', 'FontSize', 20)
% ylabel('p(change in mean spin configuration)', 'FontSize', 20)
% title('change in configuration when freezing slow spins', 'FontSize', 20)
% legend({'same drive before and after freezing', 'different drive before and after freezing'}, 'FontSize', 12)
% 
% figure(8)
% scatter(all_means_i_2, all_means_f_2, 'Filled')
% 
...
% hold on
% histogram(all_means_i_2 - all_means_i, -15:.5:15, 'Normalization', 'probability')
...
% title('change in work absorption rate when freezing slow spins', 'FontSize', 20)
% legend({'same drive before and after freezing', 'different drive before and after freezing'}, 'FontSize', 12)
