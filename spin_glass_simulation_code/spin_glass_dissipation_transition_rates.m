...
%             load(char(strcat(filename, string(iter_4), '_', string(iter_5), '/extra_data.mat')))
            ...
            interp_stats = movsum(interp_stats, 100) ./ movsum(ones(1, numel(interp_stats)), 100);
            interp_stats = interp_stats(101:(end - 100));
            transitions = diff(interp_stats > high_threshold);
            high_start = find(transitions == 1);
            low_start = find(transitions == -1);
            if(interp_stats(1) < high_threshold)
                if(interp_stats(end) > high_threshold)
                    low_start = [low_start, numel(interp_stats)];
                end
                low_times = [high_start, numel(interp_stats)] - [0, low_start];
                high_times = low_start - high_start;
            else
                
                if(interp_stats(end) < high_threshold)
                    high_start = [high_start, numel(interp_stats)];
                end
                low_times = high_start - low_start;
                high_times = [low_start, numel(interp_stats)] - [0, high_start];
            end
            last_initial = find(high_start < 1000);
            first_final = find(high_start > 5000);
            if(numel(high_start) < numel(high_times))
                high_start = [0, high_start];
            end
            stat_means_i{iter_4} = [stat_means_i{iter_4}, high_start];
            stat_means_f{iter_4} = [stat_means_f{iter_4}, high_times];
            stat_buckets{iter_4} = 
            
%             figure(1)
%             plot(movsum(interp_stats, 100) ./ movsum(ones(1, numel(interp_stats)), 100))
...
for iter_4 = 1:3
    figure(iter_4 + 9)
%     histogram(stat_means_i{iter_4}, 100, 'Normalization', 'probability', 'FaceColor', plot_colors(iter_4, :))
%     
%     figure(iter_4 + 5)
%     histogram(stat_means_f{iter_4}, 100, 'Normalization', 'probability', 'FaceColor', plot_colors(iter_4, :))
    scatter(stat_means_i{iter_4}, stat_means_f{iter_4} ./ (10000 - stat_means_i{iter_4}), 'MarkerEdgeColor', plot_colors(iter_4, :))
    
%     figure(iter_4)
%     histogram(stat_means_i{iter_4}, 25, 'Normalization', 'probability', 'FaceColor', plot_colors(iter_4, :))
%     title('initial dissipation rate')
%     figure(iter_4 + 5)
%     histogram(stat_means_f{iter_4}, 25, 'Normalization', 'probability', 'FaceColor', plot_colors(iter_4, :))
