...
filename = 'spin_glass_switch_fields_rand_adj_num_fields_2017_07_13_10_23/random_driving_';
...
field_order = mod(cumsum(random_order), num_driving_fields);
field_order(field_order == 0) = num_driving_fields;

field_num = 1;
relevant_fields = find(field_order == field_num);
relevant_times = 1 + (relevant_fields * switch_time / save_time);
% relevant_times(relevant_times > 10000) = [];
% hists = zeros(1, numel(relevant_fields) - 1);

mean_rates = [];
std_rates = [];


plot_colors = [linspace(0, 1, 15); zeros(1, 15); linspace(1, 0, 15)]';
...
for iter_3 = 1:1
    for iter_4 = 1:17
        ...
        relevant_fields = find(field_order == field_num);
        relevant_times = 1 + (relevant_fields * switch_time / save_time);
        relevant_times = relevant_times(1:(end - 1));
        
        hists = zeros(1, numel(relevant_times));
        mean_diffs = zeros(1, numel(relevant_times) - 1);
        all_data = [];
        statistic = [];
        for iter_5 = 1:10
            ...
            relevant_data = hist(slow_spins, relevant_times);
            times = stats(1, relevant_times);
...
%             mean_rates = [mean_rates, mean(mean_diff .* diff(times))];
        ...
        std_rates = [std_rates, std(mean_diffs .* diff(times))];
...
% %         plot(hists(1:50, :)')
% % %         plot(std_dev);
% %         title('random driving', 'FontSize', 20)
% %         xlabel('t', 'FontSize', 20)
% %         ylabel('<s_i>', 'FontSize', 20)
%         title('random driving pairwise change rate')
%         
...
%         figure(10)
%         hold on
%         random_plot = plot([0, times(2:end)], [0, cumsum(mean_diffs .* diff(times))], 'LineWidth', 1, 'Color', [1, 0, 0]);
%         legend([periodic_plot, random_plot], {'periodic driving', 'random driving'}, 'FontSize', 20)
%         xlabel('time', 'FontSize', 20)
%         ylabel('total configuration change', 'FontSize', 20)
%         title('Configuration Change over Time', 'FontSize', 20)
%         
...
%         
%         hold on
%         random_plot_2 = plot(new_times(2:end), diff(interp_accum_diff) / ((times(end) - times(1)) / 100), 'LineWidth', 1, 'Color', [1, 0, 0]);
        
    end
end

xlabel('time', 'FontSize', 20)
ylabel('pairwise configuration distance change rate', 'FontSize', 20)
% legend([periodic_plot_2, random_plot_2], {'periodic driving', 'random driving'}, 'FontSize', 20)
title('Configuration Change Rate over Time', 'FontSize', 20)
...
errorbar(num_fields, mean_rates, std_rates, 'Color', [1, 0, 0])
legend({'periodic driving', 'random driving'}, 'FontSize', 20)
xlabel('mean pairwise configuration distance', 'FontSize', 20)
ylabel('p(mean pairwise configuration distance)', 'FontSize', 20)
title('Distribution of Configuration Change Rates', 'FontSize', 20)

figure(8)
hold on
errorbar(num_fields, mean_rates ./ num_fields, std_rates ./ num_fields, 'Color', [1, 0, 0])
