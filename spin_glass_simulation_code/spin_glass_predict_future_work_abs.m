...
filename = 'spin_glass_switch_fields_drive_all_pent_2019_04_03_12_12/random_order_';
...
% fprintf(stat_id, '%u\n', num_extract * 100);
mean_expected_work_abs = zeros(1, 300);
mean_possible_work_abs = zeros(1, 300);
...
%                     hist_2 = [hist_2, spin_hist];
            ...
            for next_t = 100:100:30000
                ...
                field_nums = 1:size(driving_fields, 2);
                field_nums(field_number) = [];
                current_expected_work_abs = 0;
                current_possible_work_abs = 0;
                for field_num = field_nums
                    work = spin_glass_function_switch_driving_fields_return_work(state, logical(zeros(num_spins,1)), interactions, spin_barriers, 0, beta, driving_fields, switch_time, [field_number, field_num, field_number], 0, 200, 'something_went_wrong.m');
                    current_expected_work_abs = current_expected_work_abs + work;
                    if abs(field_num - field_number) == 1 || abs(field_num - field_number) == 8
                        current_possible_work_abs = current_possible_work_abs + work;
                    end
                end
                expected_work_abs = [expected_work_abs, current_expected_work_abs];
                possible_work_abs = [possible_work_abs, current_possible_work_abs];
            end
            
            mean_expected_work_abs = mean_expected_work_abs + expected_work_abs / (size(driving_fields, 2) - 1);
            mean_possible_work_abs = mean_possible_work_abs + possible_work_abs / 2;
            ...
            all_early = [all_early, early_counts];
            all_late = [all_late, late_counts];
        end
    end
end
...
figure()
plot(mean_expected_work_abs, 'LineWidth', 2)
hold on
plot(mean_possible_work_abs, 'LineWidth', 2)
