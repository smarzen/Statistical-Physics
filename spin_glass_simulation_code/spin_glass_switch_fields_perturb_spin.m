...
%folder_name = strcat('spin_glass_switch_fields_perturb_slow_', string(date));
%mkdir(char(folder_name))

%REMEMBER THAT TEMPERATURE OF THE EQUILIBRATION SIMULATIONS CURRENTLY HAS
%BETA EQUAL TO 1

for total_iter = 1:5

date = datetime('now', 'Format', 'yyyy_MM_dd_h_mm');
folder_name = strcat('spin_glass_switch_fields_perturb_slow_many_drives_', string(date));
mkdir(char(folder_name))
...
spin_barriers(randperm(num_spins, floor(num_spins / 2))) = 0 + .5 * (total_iter);
...
driving_fields = zeros(num_spins, 5);
...
while numel(random_order) < 10001
    random_order = [random_order, randperm(5)];
end
random_order = diff(random_order);

random_order_2 = [];
while numel(random_order_2) < 10001
    random_order_2 = [random_order_2, randperm(5)];
end
...
for iter = 1:100
    driving_fields(rand_fast_spins, :) = normrnd(0, 2, [numel(rand_fast_spins), 5]);
    
    
    for iter_2 = 1:10
        %driving_fields(rand_fast_spins, :) = normrnd(0, 2, [numel(rand_fast_spins), 5]);
        
        file = strcat('fixed_slow_', string(iter), '_', string(iter_2), '_', string(0));
        final_spins_fixed = spin_glass_function_switch_driving_fields_fix_spins(final_spins(:, 1), logical(fixed_spins), interactions, spin_barriers, 0, 10, driving_fields, 100, random_order, 0, 100000, strcat(folder_name, '/', file));
        
        file_3 = strcat('unfixed_slow_', string(iter), '_', string(iter_2), '_', string(0));
...
%         semi_random_order(randperm(numel(semi_random_order), floor(numel(semi_random_order) / 10))) = 2;
        ...
        for iter_3= 1:5
%         for iter_3 = 1:floor(num_spins/2)
            file_2 = strcat('fixed_slow_', string(iter), '_', string(iter_2), '_', string(iter_3));
%             fixed_spins = zeros(num_spins, 1);
%             fixed_spins(iter_3) = 1;
            modified_spins = final_spins_fixed;
            modified_spins(iter_3) = -1 * modified_spins(iter_3);
            final_spins_fixed = spin_glass_function_switch_driving_fields_fix_spins(modified_spins, logical(fixed_spins), interactions, spin_barriers, 0, 10, driving_fields, 100, random_order_2, 0, 100000, strcat(folder_name, '/', file_2));
            
            file_4 = strcat('unfixed_slow_', string(iter), '_', string(iter_2), '_', string(iter_3));
