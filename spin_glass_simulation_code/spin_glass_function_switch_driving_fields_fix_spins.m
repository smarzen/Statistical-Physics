function final_spins = spin_glass_function_switch_driving_fields_fix_spins(spins, fixed_spins, interactions, spin_barriers, external_field, beta, driving_fields, switch_time, random_order, driving_strength, t_max, file_name)
...
%             spin_work_hist = [spin_work_hist, total_spin_work];
            ...
            if iterations_since_last_save >= iterations_per_save
                save(char(strcat(file_name, '/data_', num2str(file_num))), 'statistics', 'spin_dissipation', 'spin_hist', 'flip_counts')%, 'force_vecs')%, 'spin_hist', 'flip_counts', 'spin_dissipation')
                iterations_since_last_save = 0;
                statistics = [];
                spin_hist = [];
                spin_dissipation = [];
