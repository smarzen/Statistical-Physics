function final_spins = spin_glass_function_two_baths(spins, fixed_spins, interactions, spin_barriers, external_field, beta, beta_2, bath_2_spins, driving_fields, switch_time, random_order, driving_strength, t_max, file_name)
...
flip_probability_2 = exp((total_energy - spin_barriers) * beta_2);
flip_probability_2(bath_2_spins == 0) = 0;
flip_rates_2 = zeros(ceil(log2(num_spins)), 2^ceil(log2(num_spins)));
for i = 1:num_spins
    index = 1;
    remainder = i;
    for j = 1:size(flip_rates_2, 1)
        index = index + floor((remainder - 1) / 2^(size(flip_rates_2, 1) - j)) * 2^(size(flip_rates_2, 1) - j);
        remainder = remainder - floor((remainder - 1) / 2^(size(flip_rates_2, 1) - j)) * 2^(size(flip_rates_2, 1) - j);
        flip_rates_2(j, index) = flip_rates_2(j, index) + flip_probability(i);
    end
end
total_flip_rate_2 = flip_rates_2(2, 2) + flip_rates_2(1, floor(size(flip_rates_2, 2) / 2) + 1);
...
% force_vecs = [];
statistics = [t; energy; internal_energy; mean_mag; total_work; total_heat_lost; total_heat_lost_2; internal_work; hamming_dist; slow_energy];
...
while t < t_max
    ...
    delta_t = log(1/ (1 - rand(1))) / (total_flip_rate + total_flip_rate_2);
...
%             force_vec = zeros([num_spins, 1]);
            ...
            last_config = spins;
            statistics = [statistics, [t; energy; internal_energy; mean_mag; total_work; total_heat_lost; total_heat_lost_2; internal_work; hamming_dist; slow_energy]];
            for field_iter = 1:size(driving_fields, 2)
...
%         force_vec = force_vec - total_energy .* (total_energy < 0) .* spins * delta_t;
        ...
        bath_rand = rand(1);
        if bath_rand < total_flip_rate / (total_flip_rate + total_flip_rate_2)
    ...
    %                 neighbor_dist = [neighbor_dist, new_neighbor_dist];
        ...
        %                     old_rate = flip_rates(end, neighbor_num);
                            ...
                            new_rate_2 = exp((local_energy(neighbor_num) + driving_energy(neighbor_num) - spin_barriers(neighbor_num)) * beta_2) * bath_2_spins(neighbor_num);
                            flip_rates(end, neighbor_num) = new_rate;
                            flip_rates_2(end, neighbor_num) = new_rate_2;
                            for j = 1:(size(flip_rates, 1) - 1)
                                ...
                                flip_rates_2(end - j, neighbor_num) = flip_rates_2(end - (j - 1), neighbor_num) + flip_rates_2(end - (j - 1), neighbor_num + 2^(j - 1));
                            end
        ...
        %                     end
                    ...
                    total_flip_rate = flip_rates(1, 1) + flip_rates(1, floor(size(flip_rates, 2) / 2) + 1);
                    total_flip_rate_2 = flip_rates_2(1, 1) + flip_rates_2(1, floor(size(flip_rates, 2) / 2) + 1);
    ...
    %                 driving_times(flip_spin, driving_count(flip_spin) - prev_driving_count(flip_spin) + 1) = t;
            ...
            index = 1;
            prob = 0;
            for i = 1:size(flip_rates_2, 1)
                binary_prob = flip_rates_2(i, index) / total_flip_rate_2;
                if prob + binary_prob < flip_rand
                    index = index + 2^(size(flip_rates_2, 1) - i);
                    prob = prob + binary_prob;
                end
            end
            flip_spin = index;
    ...
    %                 end
                    ...
                    total_heat_lost_2 = total_heat_lost_2 - 2 * local_energy(flip_spin) - 2 * driving_energy(flip_spin);
                    total_spin_dissipation(flip_spin) = total_spin_dissipation(flip_spin) - 2 * local_energy(flip_spin) - 2 * driving_energy(flip_spin);
                    if driving_on(flip_spin)
                        internal_work = internal_work + 2 * local_energy(flip_spin);
                    end
