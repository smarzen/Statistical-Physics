...
filename = 'spin_glass_switch_fields_eff_temp_glassy_2017_08_16_3_27/driving_disabled_';
...
final_stats = [];
final_std = [];
plot_colors = [linspace(0, 1, 12); zeros(1, 12); linspace(1, 0, 12)]';
...
%             load(char(strcat(filename, string(iter_3), '_', string(iter_4), '_', string(iter_5), '/extra_data.mat')))
            ...
            statistic = [statistic; interp1(stats(1, t_index), stats(3, t_index), 0:100:t_max)];
            %t, energy, internal energy, mean mag, work, heat lost, internal work
        end
        figure(6)
        hold on
%         plot(0:100:(t_max - 100), mean(diff(statistic, 1, 2), 1), 'Color', plot_colors(iter_4, :));
...
errorbar([1, 2, 3, 5, 7, 10], final_stats, final_std, 'Color', [0, 0, 1])
