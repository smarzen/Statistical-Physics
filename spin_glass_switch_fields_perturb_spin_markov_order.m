...
folder_name = strcat('spin_glass_switch_fields_markov_order_', string(date));
...
interaction_locs = 1 * (double(interaction_locs) .* (normrnd(0, .625, size(interaction_locs)))); %.625 for 32 * 32
...
%             driving_fields_2 = zeros(num_spins, number_fields(iter_3));
            ...
            driving_fields(:, :) = normrnd(0, 1, [num_spins, 6]); %1 for 32 * 32
...
%             random_order = random_order(:);
            ...
            while numel(random_order) < 3001
                next_rand = mod(random_order(end) + randi(3) - 1, 4) + 1;
...
%             final_spins_fixed = spin_glass_function_switch_driving_fields_fix_spins(final_spins(:, 1), logical(zeros(num_spins, 1)), interactions, spin_barriers, 0, 10, driving_fields_2, 20000, random_order, 0, 100000, strcat(folder_name, '/', file_5));
            ...
            file_2 = strcat('markov_order_', string(total_iter), '_', string(iter_2), '_', string(iter_3));
