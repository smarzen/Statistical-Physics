...
folder_name = strcat('spin_glass_switch_fields_high_barrier_', string(date));
mkdir(char(folder_name))

for total_iter = 1:1000
...
spin_barriers = zeros(num_spins, 1) + .49;
...
driving_fields = zeros(num_spins, num_fields + num_fields_2);
driving_fields_2 = zeros(num_spins, num_fields);
driving_fields_3 = zeros(num_spins, num_fields_2);
...
while numel(random_order_3) < 10001
    random_order_3 = [random_order_3, randperm(num_fields_2)];
end
...
num_field_repeats = [1, 2, 3, 5, 10, 20, 30, 50, 100, 200];
...
%         driving_fields(rand_fast_spins, :) = normrnd(0, 2, [numel(rand_fast_spins), 1]);
        driving_fields(rand_fast_spins, :) = normrnd(0, 20, [numel(rand_fast_spins), num_fields + num_fields_2]);
        driving_fields_2(rand_fast_spins, :) = normrnd(0, 20, [numel(rand_fast_spins), num_fields]);
        driving_fields_3(rand_fast_spins, :) = normrnd(0, 20, [numel(rand_fast_spins), num_fields_2]);
...
%         semi_random_order(randperm(numel(semi_random_order), floor(numel(semi_random_order) / 10))) = 2;
        ...
        for iter_3= 1:10
            random_order = [];
            while numel(random_order) < 10001
                for iter_4 = 1:num_field_repeats(iter_3)
                    random_order = [random_order, randperm(num_fields)];
                end
                for iter_4 = 1:num_field_repeats(iter_3)
                    random_order = [random_order, randperm(num_fields_2) + num_fields];
                end
            end
            random_order = diff(random_order);
            ...
            while numel(random_order_2) < 10001
                ...
                for iter_4 = 1:num_field_repeats(iter_3)
                    random_order_2 = [random_order_2, randperm(num_fields_2) + num_fields];
                end
            end
            random_order_2 = diff(random_order_2);
...
%             final_spins_fixed = spin_glass_function_switch_driving_fields_fix_spins(final_spins(:, 1), logical(zeros(num_spins, 1)), interactions, spin_barriers, 0, 10, driving_fields_2, 20000, random_order, 0, 100000, strcat(folder_name, '/', file_5));
            ...
            file = strcat('change_drive_time_', string(total_iter), '_', string(iter_2), '_', string(iter_3));
            final_spins_same = spin_glass_function_switch_driving_fields_fix_spins(final_spins(:, 1), logical(zeros(num_spins, 1)), interactions, spin_barriers, 0, 10, driving_fields, 100, random_order, 0, 100000, strcat(folder_name, '/', file));
...
%             
            file_2 = strcat('change_drive_time_', string(total_iter), '_', string(iter_2), '_', string(iter_3), 'b');
...
%             modified_spins(1) = -1 * modified_spins(1);
            final_spins_same = spin_glass_function_switch_driving_fields_fix_spins(modified_spins, logical(fixed_spins), interactions, spin_barriers, 0, 10, driving_fields, 100, random_order_2, 0, 100000, strcat(folder_name, '/', file_2));
            
%             file_4 = strcat('decorr_second_', string(total_iter), '_', string(iter_2), '_', string(iter_3));
% %             modified_spins = final_spins_diff;0
% %             modified_spins(1) = -1 * modified_spins(1);
%             final_spins_diff = spin_glass_function_switch_driving_fields_fix_spins(final_spins(:, iter_3), logical(zeros(num_spins, 1)), interactions, spin_barriers, 0, 10, driving_fields, 100, random_order, 0, 100000, strcat(folder_name, '/', file_4));
%             
%             file_5 = strcat('decorr_second_', string(total_iter), '_', string(iter_2), '_', string(iter_3), 'b');
%             final_spins_fixed_2 = spin_glass_function_switch_driving_fields_fix_spins(final_spins_diff, logical(zeros(num_spins, 1)), interactions, spin_barriers, 0, 10, driving_fields, 100, random_order_2_b, 0, 100000, strcat(folder_name, '/', file_5));
%             
