...
filename = 'spin_glass_switch_fields_drive_all_per_2019_05_15_1_14/random_order_';
...
for transition_start = 1:5
    for transition_end = 1:5
        if transition_start ~= transition_end
            ...
            %                     hist_2 = [hist_2, spin_hist];
                        ...
                        later_diss = zeros(1, 256);
                        latest_diss = zeros(1, 256);
                        ...
                        trans_count = 0;
                        for field_t = 1:299
                            if random_order(field_t) == transition_start && random_order(field_t + 1) == transition_end
                                if first_t == 0
                                    first_t = field_t;
                                elseif second_t == 0
                                    second_t = field_t;
                        ...
                        if trans_count >= 4
                            ...
                            for next_t = switch_time * (first_t - 1)
                            ...
                            for next_t = switch_time * first_t
                            ...
                            for next_t = switch_time * (second_t - 1)
                                while iter_6 <= numel(flip_times) && flip_times(iter_6) < next_t
                                    state(flip_order(iter_6)) = state(flip_order(iter_6)) * -1;
                                    iter_6 = iter_6 + 1;
                                end
                            ...
                            for next_t = switch_time * second_t
                                while iter_6 <= numel(flip_times) && flip_times(iter_6) < next_t
                                    field_number = random_order(floor(flip_times(iter_6) / switch_time) + 1);
                                    spin_num = flip_order(iter_6);
                                    state(flip_order(iter_6)) = state(flip_order(iter_6)) * -1;
                            ...
                            for next_t = switch_time * (penult_t - 1)
                                while iter_6 <= numel(flip_times) && flip_times(iter_6) < next_t
                                    state(flip_order(iter_6)) = state(flip_order(iter_6)) * -1;
                                    iter_6 = iter_6 + 1;
                                end
                            ...
                            for next_t = switch_time * penult_t
                                while flip_times(iter_6) < next_t
                                    field_number = random_order(floor(flip_times(iter_6) / switch_time) + 1);
                                    spin_num = flip_order(iter_6);
                                    state(flip_order(iter_6)) = state(flip_order(iter_6)) * -1;
                                    early_counts(flip_order(iter_6)) = early_counts(flip_order(iter_6)) + 1;
                                    later_diss(spin_num) = later_diss(spin_num) - 2 * state(spin_num) * (interactions(spin_num, :) * state + driving_fields(spin_num, field_number));
                                    iter_6 = iter_6 + 1;
                                end
                            ...
                            for next_t = switch_time * (last_t - 1)
                                while iter_6 <= numel(flip_times) && flip_times(iter_6) < next_t
                                    state(flip_order(iter_6)) = state(flip_order(iter_6)) * -1;
                                    iter_6 = iter_6 + 1;
                                end
                            ...
                            for next_t = switch_time * last_t
                                while flip_times(iter_6) < next_t
                                    field_number = random_order(floor(flip_times(iter_6) / switch_time) + 1);
                                    spin_num = flip_order(iter_6);
                                    state(flip_order(iter_6)) = state(flip_order(iter_6)) * -1;
                                    early_counts(flip_order(iter_6)) = early_counts(flip_order(iter_6)) + 1;
                                    latest_diss(spin_num) = latest_diss(spin_num) - 2 * state(spin_num) * (interactions(spin_num, :) * state + driving_fields(spin_num, field_number));
                                    iter_6 = iter_6 + 1;
                                end
                ...
                %             late_diss = sort(late_diss);
                            ...
                            no_change = find((all_early == all_late) & (all_late == all_later) & (all_later == all_latest));
                            all_early(no_change) = [];
                            all_late(no_change) = [];
                            all_later(no_change) = [];
                            all_latest(no_change) = [];

                            all_early = [all_early, early_diss];
                            all_late = [all_late, late_diss];
                            all_later = [all_later, later_diss];
                            all_latest = [all_latest, latest_diss];
                        end
                    end
                end
            end
...
diss_a = all_early;
diss_b = all_late;
no_change = find(diss_a == diss_b);
diss_a(no_change) = [];
diss_b(no_change) = [];

figure()
histogram(diss_b - diss_a, -50:.5:50)
