...
filename = 'spin_glass_switch_fields_record_force_2017_09_27_5_33/periodic_driving_';
...
stat_mean = [];
stat_sq_mean = [];
force_vector_means_i = {};
force_vector_means_f = {};
for iter_4 = 1:5
    force_vector_means_i{iter_4} = [];
    force_vector_means_f{iter_4} = [];
end
...
for iter_3 = 1:50
    for iter_4 = 1:5
        for iter_5 = 1:5
            ...
            force_vector_mean_i = mean(sqrt(sum(force_vector(slow_spins, 1:1000).^2, 1)));
            force_vector_mean_f = mean(sqrt(sum(force_vector(slow_spins, (end - 999):end).^2, 1)));
            force_vector_means_i{iter_4} = [force_vector_means_i{iter_4}, force_vector_mean_i];
            force_vector_means_f{iter_4} = [force_vector_means_f{iter_4}, force_vector_mean_f];
...
figure(iter_4 + 5)
histogram(force_vector_means_f{iter_4} - force_vector_means_i{iter_4}, 25, 'Normalization', 'probability', 'FaceColor', plot_colors(:, iter_4))

% figure(6)
% histogram(force_vector_means_f{iter_4}, 25, 'Normalization', 'probability', 'FaceColor', plot_colors(:, iter_4))
