...
all_local_energy_init = [];
all_local_energy_final = [];
...
bin_edge_row = -10:.5:10;
bin_edge_col = -16:.5:5;
bin_count_init = zeros(numel(bin_edge_row), numel(bin_edge_col));
bin_count_final = zeros(numel(bin_edge_row), numel(bin_edge_col));
bin_flip_init = zeros(numel(bin_edge_row), numel(bin_edge_col));
bin_flip_final = zeros(numel(bin_edge_row), numel(bin_edge_col));
...
%             all_local_energy_init = [all_local_energy_init; max(abs(driving_fields(:, 1:5)), [], 2)];
            ...
            drive_energy_init = driving_fields(:, random_order(10)) .* state;
            
            for next_t = 2000:1000:29000
            ...
            drive_energy_final = driving_fields(:, random_order(300)) .* state;
            
%             all_local_energy_final = [all_local_energy_final; mean(local_energy_list(:, (end - 4):end), 2)];
...
%             [late_counts, late_count_indices] = sort(late_counts);
            ...
            for iter_6 = 1:31
                local_energy_trajectory(iter_6) = local_energy_trajectory(iter_6) + local_energy_list(late_count_indices(spin_of_interest), iter_6);
            end
            
            for iter_6 = 1:num_spins
                if numel(interactions(:, iter_6) ~= 0) > 0
                    row = find(drive_energy_init(iter_6) < bin_edge_row);
                    row = row(1);
                    col = find(local_energy_list(iter_6, 2) < bin_edge_col);
                    col = col(1);
                    bin_count_init(row, col) = bin_count_init(row, col) + 1;
                    bin_flip_init(row, col) = bin_flip_init(row, col) + early_counts(iter_6);

                    row = find(drive_energy_final(iter_6) < bin_edge_row);
                    row = row(1);
                    col = find(local_energy_list(iter_6, end) < bin_edge_col);
                    col = col(1);
                    bin_count_final(row, col) = bin_count_final(row, col) + 1;
                    bin_flip_final(row, col) = bin_flip_final(row, col) + late_counts(iter_6);
                end
            end
...
figure()
surf(bin_edge_col, bin_edge_row, bin_count_init)
xlabel('local energy', 'FontSize', 20)
ylabel('drive energy', 'FontSize', 20)
title('histogram of spin energies in initial config', 'FontSize', 20)
axis([-16, 5, -10, 10])
colorbar

figure()
surf(bin_edge_col, bin_edge_row, bin_count_final)

figure()
surf(bin_edge_col, bin_edge_row, bin_count_final - bin_count_init)
xlabel('local energy', 'FontSize', 20)
ylabel('drive energy', 'FontSize', 20)
title('change in histogram of spin energies from initial to final config', 'FontSize', 20)
axis([-16, 5, -10, 10])
colorbar

bin_count_init_pad = bin_count_init;
bin_count_init_pad(bin_count_init_pad == 0) = 1;
bin_count_final_pad = bin_count_final;
bin_count_final_pad(bin_count_final_pad == 0) = 1;

figure()
surf(bin_edge_col, bin_edge_row, bin_flip_init ./ bin_count_init_pad)
xlabel('local energy', 'FontSize', 20)
ylabel('drive energy', 'FontSize', 20)
title('flips per 1000s in initial config', 'FontSize', 20)
axis([-16, 5, -10, 10])
colorbar

figure()
surf(bin_edge_col, bin_edge_row, bin_flip_final ./ bin_count_final_pad)

figure()
surf(bin_edge_col, bin_edge_row, bin_flip_final./ bin_count_final_pad - bin_flip_init ./ bin_count_init_pad)

bin_flip_init_adjust = bin_flip_init;
bin_flip_init_adjust(bin_count_init < 30) = 0;
bin_flip_init_adjust(bin_count_final < 30) = 0;

bin_flip_final_adjust = bin_flip_final;
bin_flip_final_adjust(bin_count_init < 30) = 0;
bin_flip_final_adjust(bin_count_final < 30) = 0;

figure()
surf(bin_edge_col, bin_edge_row, bin_flip_final_adjust./ bin_count_final_pad - bin_flip_init_adjust ./ bin_count_init_pad)
xlabel('local energy', 'FontSize', 20)
ylabel('drive energy', 'FontSize', 20)
title('change in flips per 1000s from initial to final config', 'FontSize', 20)
axis([-16, 5, -10, 10])
colorbar
...
% figure()
% yyaxis left
% plot(1000:1000:30000, flip_trajectory / num_extract, 'LineWidth', 2)
% xlabel('t', 'FontSize', 20)
% ylabel('flips per 1000s', 'FontSize', 20)
% title('increased temp rank 225 final flip rate', 'FontSize', 20)
% 
% yyaxis right
...
% figure()
% histogram(all_local_energy_init)
% hold on
% histogram(all_local_energy_final)
% 
% figure()
% histogram(all_local_energy_final - all_local_energy_init)

% figure()
% scatter(all_local_energy_init, all_local_energy_final)
