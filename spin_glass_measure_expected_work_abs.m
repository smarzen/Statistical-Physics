...
filename = 'spin_glass_switch_fields_drive_quarter_reverse_field_order_temp_sweep_2019_01_30_3_15/random_order_';
...
mean_work_abs = zeros(1, 290);
mean_other_abs = zeros(1, 290);
            ...
            for iter_7 = 1:290
                ...
                for field_num = 1:10
                    if field_num ~= current_field
                        potential_work = driving_fields(:, field_num) .* state - driving_fields(:, current_field) .* state;
                        current_energy = state .* interactions * state + driving_fields(:, field_num) .* state;
                        potential_work(current_energy < 0) = 0;
                        if transitions(random_order(iter_7), field_num)
                            next_energy = next_energy + sum(potential_work);
                        else
                            other_energy = other_energy + sum(potential_work);
                        end
                    end
                end
